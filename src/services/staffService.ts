...
export async function getStaffMembers(
  ...
  const response = await fetch(
    `${API_BASE}/api/v1/staff?page=1&limit=10&active_only=true`,
    {
      method: "GET",
...
export async function createAvailability(
  ...
  const response = await fetch(
    `${API_BASE}/api/v1/staff/${staffId}/availability`,
    {
      method: "POST",
      headers: getAdminHeaders(),
 ...
 */
export async function getStaffBySpecialty(
  specialty: string
): Promise<StaffResponse[]> {
  const response = await fetch(
...
//   const response = await fetch(${API_BASE}/api/v1/staff?page=1&limit=10&active_only=true, {
//     method: 'GET',
//     headers: getAdminHeaders(),
//   })
